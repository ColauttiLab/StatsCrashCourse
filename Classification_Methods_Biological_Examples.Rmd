...
```{r c(dplyr,FSA,PerformanceAnalytics,psych,car,rcompanion,lmtest,aod)=FALSE}
...
setwd("~/Course_Development/02_Statistical_Learning/04_Classification/Classification_Methods")
...
iris.dat<-read.csv("Iris.data.csv", header=T)
head(iris.dat)
...
training.samples <- iris.dat$species %>% createDataPartition(p = 0.8, list = FALSE)

train.data <- iris.dat[training.samples, ]
test.data <- iris.dat[-training.samples, ]
...
partimat(species ~ s.length + s.width + p.length + p.width, data=train.transformed, method="lda")
...
# Fit the model
model.qda <- qda(species~s.length+s.width+p.length+p.width, data = train.transformed)
model.qda
...
partimat(species~s.length+s.width+p.length+p.width, data = train.transformed, method="qda")
...
##Generate a random number that is 80% of the total number of rows in dataset.
 ran <- sample(1:nrow(iris.dat), 0.8 * nrow(iris.dat)) 
 ...
 iris_norm <- as.data.frame(lapply(iris.dat[,c(2,3,4,5)], nor))
